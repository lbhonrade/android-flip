...
public class FlipViewController extends AdapterView<Adapter> {
	...
	public void preloadAllViews(int selectedPosition) {
		if (adapter == null) {
			return;
		}
		
		Assert.assertTrue("Invalid selection position", selectedPosition >= 0
				&& selectedPosition < adapterDataCount);
		
		View selectedView = viewFromAdapter(selectedPosition, true);
		...
		for (int i = 0; i < adapterDataCount; i++) {
			if(i <= selectedPosition) {
				bufferedViews.add(viewFromAdapter(i, false));
			} else if (i > selectedPosition) {
				bufferedViews.addLast(viewFromAdapter(i, true));
			}
		}

		bufferIndex = bufferedViews.indexOf(selectedView);
		adapterIndex = selectedPosition;

		requestLayout();
		updateVisibleView(inFlipAnimation ? -1 : bufferIndex);

		cards.resetSelection(selectedPosition, adapterDataCount);
	}
	...
	public void flipOnceToPosition(final int selectedPosition) {
		if (adapter == null) {
			return;
		}
		
		final int currentPosition = this.getSelectedItemPosition();
		
		Assert.assertTrue("Invalid selection position", selectedPosition >= 0
				&& selectedPosition < adapterDataCount);
		
		View selectedView = viewFromAdapter(selectedPosition, false);
		View currentView = viewFromAdapter(currentPosition, true);
		
		releaseViews();
		
		bufferedViews.add(currentView);
		if(currentPosition > selectedPosition) {
			bufferedViews.addFirst(selectedView);			
		} else {
			bufferedViews.addLast(selectedView);			
		}
		

		bufferIndex = bufferedViews.indexOf(currentView);
		adapterIndex = currentPosition;

		requestLayout();
		updateVisibleView(inFlipAnimation ? -1 : bufferIndex);
		
		cards.resetSelection(currentPosition, adapterDataCount);
